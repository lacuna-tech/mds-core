...
export async function readDevicesByVehicleId(
  ...
  const sql = `SELECT * FROM "devices" WHERE provider_id=${vals.add(
    provider_id
...
export async function readDeviceIds(provider_id?: UUID, skip?: number, take?: number): Promise<DeviceID[]> {
  // read from pg
  const client = await getReadOnlyClient()
  let sql = `SELECT device_id, provider_id FROM "devices"`
  const vals = new SqlVals()
...
export async function readDevice(
  ...
  const sql = provider_id
    ? `SELECT * FROM "devices" WHERE device_id=$1 AND provider_id=$2`
...
export async function readDeviceList(device_ids: UUID[]): Promise<Recorded<Device>[]> {
  ...
  const client = await getReadOnlyClient()
  const vals = new SqlVals()
  const sql = `SELECT * FROM "devices" WHERE device_id IN (${device_ids.map(device_id => vals.add(device_id))})`
  const values = vals.values()
  await logSql(sql, values)
  const result = await client.query(sql, values)
  return result.rows
...
export async function writeDevice(device: Device): Promise<Recorded<Device>> {
  const columns = ['device_id', 'provider_id', 'vehicle_id', 'type', 'propulsion', 'year', 'mfgr', 'model', 'recorded']
  const client = await getWriteableClient()
  const sql = `INSERT INTO "devices" (${cols_sql(columns)}) VALUES (${vals_sql(columns)}) RETURNING *`
  const values = vals_list(columns, { ...device, recorded: now() })
  await logSql(sql, values)
  const {
...
export async function updateDevice(device_id: UUID, provider_id: UUID, changes: Partial<Device>): Promise<Device> {
  ...
  const sql = `UPDATE "devices" SET vehicle_id = $1 WHERE device_id = $2`
...
export async function getVehicleCountsPerProvider(): Promise<{ provider_id: UUID; count: number }[]> {
  const sql = `select provider_id, count(provider_id) from "devices" group by provider_id`
...
export async function getNumVehiclesRegisteredLast24HoursByProvider(
  ...
  const vals = new SqlVals()
  const sql = `select provider_id, count(device_id) from "devices" where recorded > ${vals.add(
    start
