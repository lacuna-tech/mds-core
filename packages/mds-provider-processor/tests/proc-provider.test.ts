...
const getMockedState = (
  ...
  const tripStartA = ({ type, event_type, timestamp, state, trip_id, provider_id, vehicle_type } as unknown) as StateEntry
  return tripStartA
}
...
const getMockedTripEntry = (
  ...
  const telemetry = ({ timestamp, violation_count,max_violation_dist,min_violation_dist,avg_violation_dist } as unknown) as TripTelemetry
  return telemetry
}
