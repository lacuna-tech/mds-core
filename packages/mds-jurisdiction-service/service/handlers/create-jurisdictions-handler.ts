...
export const CreateJurisdictionsHandler = async (
  ...
  try {
    const entities = await JurisdictionRepository.writeJurisdictions(
      jurisdictions
        .map(({ jurisdiction_id = uuid(), timestamp = recorded, ...jurisdiction }) =>
        ...
        .map(JurisdictionDomainToEntity.mapper())
    )
    return ServiceResult(entities.map(JurisdictionEntityToDomain.mapper()))
