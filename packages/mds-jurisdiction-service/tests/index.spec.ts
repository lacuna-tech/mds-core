...
describe('Write/Read Jurisdictions', () => {
  before(async () => {
    await JurisdictionServer.startup()
  })

  it(`Write ${records} Jurisdiction${records > 1 ? 's' : ''}`, async () => {
    const [error, jurisdictions] = await JurisdictionServer.createJurisdictions(
  ...
  it(`Read ${records} Jurisdiction${records > 1 ? 's' : ''}`, async () => {
    const [error, jurisdictions] = await JurisdictionServer.getJurisdictions()
    test.value(jurisdictions).isNot(null)
  ...
  it('Write One Jurisdiction', async () => {
    const [error, jurisdiction] = await JurisdictionServer.createJurisdiction({
  ...
  it('Update One Jurisdiction (invalid jurisdiction_id)', async () => {
    const [error, jurisdiction] = await JurisdictionServer.updateJurisdiction(JURISDICTION_ID, {
  ...
  it('Update One Jurisdiction (not found)', async () => {
    const [error, jurisdiction] = await JurisdictionServer.updateJurisdiction(uuid(), {
      agency_name: 'Some New Name',
      timestamp: TODAY
    })
  ...
  it('Read Specific Jurisdiction (current version)', async () => {
    const [error, jurisdiction] = await JurisdictionServer.getJurisdiction(JURISDICTION_ID)
    test.value(jurisdiction).isNot(null)
    test.value(jurisdiction?.jurisdiction_id).is(JURISDICTION_ID)
    test.value(jurisdiction?.timestamp).is(TODAY)
    test.value(error).is(null)
  ...
  it('Read Missing Jurisdiction', async () => {
    const [error, jurisdiction] = await JurisdictionServer.getJurisdiction(uuid())
    test.value(error).isNot(null).isInstanceOf(NotFoundError)
    test.value(jurisdiction).is(null)
  })

  it('Delete One Jurisdiction', async () => {
    const [error, result] = await JurisdictionServer.deleteJurisdiction(JURISDICTION_ID)
  ...
  after(async () => {
    await JurisdictionServer.shutdown()
  })
})
