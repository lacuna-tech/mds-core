...
describe('Write/Read Jurisdictions', () => {
  before(async () => {
    await JurisdictionSericeProvider.start()
  })

  it(`Write ${records} Jurisdiction${records > 1 ? 's' : ''}`, async () => {
    const [error, jurisdictions] = await JurisdictionSericeProvider.createJurisdictions(
  ...
  it(`Read ${records} Jurisdiction${records > 1 ? 's' : ''}`, async () => {
    const [error, jurisdictions] = await JurisdictionSericeProvider.getJurisdictions()
    test.value(jurisdictions).isNot(null)
  ...
  it('Write One Jurisdiction', async () => {
    const [error, jurisdiction] = await JurisdictionSericeProvider.createJurisdiction({
  ...
  it('Update One Jurisdiction (invalid jurisdiction_id)', async () => {
    const [error, jurisdiction] = await JurisdictionSericeProvider.updateJurisdiction(JURISDICTION_ID, {
  ...
  it('Update One Jurisdiction (not found)', async () => {
    const [error, jurisdiction] = await JurisdictionSericeProvider.updateJurisdiction(uuid(), {
      agency_name: 'Some New Name',
      timestamp: TODAY
    })
  ...
  it('Read Specific Jurisdiction (current version)', async () => {
    const [error, jurisdiction] = await JurisdictionSericeProvider.getJurisdiction(JURISDICTION_ID)
    test.value(jurisdiction).isNot(null)
    test.value(jurisdiction?.jurisdiction_id).is(JURISDICTION_ID)
    test.value(jurisdiction?.timestamp).is(TODAY)
    test.value(error).is(null)
  ...
  it('Read Missing Jurisdiction', async () => {
    const [error, jurisdiction] = await JurisdictionSericeProvider.getJurisdiction(uuid())
    test.value(error).isNot(null).isInstanceOf(NotFoundError)
    test.value(jurisdiction).is(null)
  })

  it('Delete One Jurisdiction', async () => {
    const [error, result] = await JurisdictionSericeProvider.deleteJurisdiction(JURISDICTION_ID)
  ...
  after(async () => {
    await JurisdictionSericeProvider.stop()
  })
})
