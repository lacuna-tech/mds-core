...
export class CreateGeographyMetadataTable1602790946354 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (geography_metadata === undefined) {
      ...
      await queryRunner.query(
        `ALTER TABLE "geography_metadata" ADD CONSTRAINT "fk_geography_id_geographies" FOREIGN KEY ("geography_id") REFERENCES "geographies"("geography_id") ON DELETE CASCADE ON UPDATE NO ACTION`
      )
    }
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE "geography_metadata" DROP CONSTRAINT "fk_geography_id_geographies"`)
